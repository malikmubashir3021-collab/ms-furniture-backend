...
router.get('/data/categories', async (_req, res: Response) => {
  const db = await getDb()
...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  const db = await getDb()
  db.run('INSERT INTO products (name, category, description, material, finishing, sizing, color_scheme, top_type, model_number, badge, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [name, category, description || '', material || '', finishing || '', sizing || '', color_scheme || '', top_type || '', model_number || '', badge || '', image || ''])
  saveDb()
...
router.put('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  const { name, category, description, material, finishing, sizing, color_scheme, top_type, model_number, badge, image } = req.body
  db.run(`UPDATE products SET name=?, category=?, description=?, material=?, finishing=?, sizing=?, color_scheme=?, top_type=?, model_number=?, badge=?, image=?, updated_at=datetime('now') WHERE id=?`,
    [name, category, description, material, finishing, sizing, color_scheme, top_type, model_number, badge, image, Number(req.params.id)])
  saveDb()
  const rows = queryAll(db, 'SELECT * FROM products WHERE id = ?', [Number(req.params.id)])
  res.json(rows[0])
})
